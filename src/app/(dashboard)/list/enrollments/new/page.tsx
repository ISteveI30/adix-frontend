...
interface EnrollmentFormProps {
  ...
  data?: {
    tutor?: Tutor
    student?: Student
    academic?: Partial<EnrollmentFormInputs>
  }
  onSuccess?: () => void
}
...
export default function EnrollmentForm({
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        ...
        if (data?.academic?.areaId) {
          const careersData = await getCareersByArea(data.academic.areaId)
  ...
  const ensureStudentExists = async (): Promise<string | undefined> => {
    // Si ya tenemos studentId y datos del tutor/estudiante, lo usamos
    if (studentId && tutorStudentData) {
      return studentId;
    }

    // Si no hay datos del tutor/estudiante, mostramos error
    if (!tutorStudentData) {
    ...
    try {
      // Guardamos primero el tutor
      let savedTutor = tutorStudentData.tutor;
      // Si el tutor no tiene ID, lo guardamos y obtenemos el ID
      if (!savedTutor.id) {
        savedTutor = await TutorService.saveTutor(tutorStudentData.tutor);
        if (!savedTutor.id) throw new Error("No se pudo obtener el ID del tutor");
      } else {
        await TutorService.updateTutor(savedTutor);
      }

      // Luego el estudiante con referencia al tutor
  ...
  const handleTutorStudentSave = async (data: { tutor: Tutor; student: Student }) => {
    try {
      ...
      console.log("Datos de tutor y estudiante:", JSON.stringify(data, null, 2))

      setStudentId(data.student.id!)

      console.log("Tutor a guardar:", JSON.stringify(tutorStudentData, null, 2));
      console.log("Student ID set:", studentId)
      ...
      // toast.success("Datos de estudiante guardados")
    } catch (error) {
  ...
  const onSubmit = async (formData: EnrollmentFormInputs) => {
    try {
      // Paso 1: Asegurar que el estudiante existe
      ...
      // Paso 2: Preparar datos de matrícula
      ...
      <Card className="shadow-lg">
        <CardHeader className="border-b">
          <CardTitle className="text-2xl font-bold text-primary">
            {type === "create" ? "Nueva Matrícula" : "Editar Matrícula"}
          </CardTitle>
        </CardHeader>
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-end pt-4">
                <Button
                  type="button"
                  variant="outline"
                  onClick={onSuccess} // O cualquier otra acción de cancelación
                >
                  Cancelar
                </Button>
              </div>
